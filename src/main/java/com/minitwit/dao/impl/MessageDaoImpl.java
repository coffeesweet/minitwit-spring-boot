...
public class MessageDaoImpl implements MessageDao {
  ...
  public List<Message> getUserFullTimelineMessages(User user) {
    ...
    String sql =
        "select message.*, user.* from message, user "
            + "where message.author_id = user.user_id and ( " + "user.user_id = :id or "
            + "user.user_id in (select followee_id from follower " + "where follower_id = :id))"
            + "order by message.pub_date desc";
    List<Message> result = template.query(sql, params, messageMapper);
  ...
  public void insertMessage(Message m) {
    ...
    String sql =
        "insert into message (author_id, text, pub_date) values (:userId, :text, :pubDate)";
